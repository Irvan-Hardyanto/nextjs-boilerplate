  ...
  return (
    <div id="tab-container" className="flex flex-col">
        ...
        {menus.map((m,idx)=>{
          return (
            <Button
                      key={idx}
              data-index={idx}
              onClick={(e)=>setActiveTab(Number(e.currentTarget.dataset.index))}
      ...
      <div id="tab-content">
        {
          menus.map((m,idx)=>(
            <div id={idx === activeTab ? "active" : "inactive" } data-index={idx} key={idx} style={{display: idx === activeTab ? "block" : "none"}} className="transition-colors">
              {m.content}
            </div>
          ))
        }
